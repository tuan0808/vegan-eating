...
//   --start N        begin at the Nth eligible recipe (1-based; for resuming on another machine)
//   --limit N        process at most N recipes (from --start onward)
...
async function main() {
    ...
    const work = eligible.slice(startIdx, limit > 0 ? startIdx + limit : eligible.length);
    ...
    console.log(`Recipes this run: ${work.length} (starting at #${start} of ${eligible.length}${limit ? `, --limit ${limit}` : ""})`);
    ...
    if (dry) {
        work.forEach((r, i) =>
            console.log(`  [${start + i}/${eligible.length}] ${r.slug}  (${pickMode(r.image)}, ${extractSteps(r.steps).length} steps)`)
        );
    ...
    for (let idx = 0; idx < work.length; idx++) {
        ...
        process.stdout.write(`[${start + idx}/${eligible.length}] ${r.slug} (${mode}) `);
        try {
